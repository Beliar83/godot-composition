...
{
    private static void ConstructProperties(ICollection<string> occupiedNames,
...
//                                       );
            ...
            string getter = 
                gdeTypeNames.Contains(castTypeName) 
                    ? $"{castTypeName}.{VariantToInstanceMethodName}({backing}Get({cachedNativeName}).AsGodotObject())" 
                    : $"({castTypeName}){backing}Get({cachedNativeName}){enumString}";
